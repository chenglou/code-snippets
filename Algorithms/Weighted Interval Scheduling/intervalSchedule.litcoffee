    ...
    recursivelySchedule = (taskNo, tasks, memoizer = {}) ->
        if taskNo is 0 then 0
            ...
            memoizer[taskNo] = Math.max tasks[taskNo].duration +
                                        recursivelySchedule(tasks[taskNo].previousNonOverlappingTask, tasks, memoizer),
                                        recursivelySchedule taskNo - 1, tasks, memoizer

    iterativelySchedule = (taskNo, tasks) ->
        memoizer = [0]
        for i in [1..taskNo]
            memoizer[i] = Math.max tasks[i].duration + memoizer[tasks[i].previousNonOverlappingTask],
                            memoizer[i - 1]
        return memoizer[memoizer.length - 1]
